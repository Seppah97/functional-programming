...
findEntries nameFind phonebook = filter (\entry -> nameFind == name entry) phonebook
...
addEntry newName phonetype ccode phonenum ccodelist currentbook = 
    let newPhone = readPhone phonetype ccode phonenum ccodelist
        newEntry = PhoneBookEntry newName newPhone
    in if any (\entry -> name entry == newName && phoneNo (phone entry) == phoneNo newPhone) currentbook
       then currentbook
       else newEntry : currentbook
